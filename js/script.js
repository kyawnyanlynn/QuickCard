...
function showSlide(index) {
  ...
  if (window.innerWidth <= 480) {
    const existingH3 = document.querySelector(
...
setInterval(() => {
  ...
  currentSlideIndex =
    (currentSlideIndex + 1) % Math.min(slidesLength, buttonsLength);
...
function updateImageSources() {
  ...
  const worriesImages = document
    .querySelector(".worries-container")
    ?.querySelectorAll("img[data-mobile-src]"); // Images within worries-container
  ...
  //max-width: 1024px (affect only worries-container images)
    ...
    allImages.forEach((img) => {
      if (!img.closest(".worries-container")) {
        // Exclude worries-container images
